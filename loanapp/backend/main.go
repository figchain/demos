...
func main() {
	...
	// Start server
	port := ":8080"
	log.Printf("Starting LoanApp API server on %s", port)
...
func initFigChain(configPath string) {
	...
	c.RegisterListener(figKey, &figchain.LoanAppParameters{}, func(r client.AvroRecord) {
		params, ok := r.(*figchain.LoanAppParameters)
		if !ok {
			log.Printf("Received record is not of type *figchain.LoanAppParameters")
			return
		}
	...
	var initialVal figchain.LoanAppParameters
